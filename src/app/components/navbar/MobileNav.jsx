...
export default function MobileNav({ isMenuOpen, setIsMenuOpen  , session}) {
    ...
    return (
        <div>
            <div
                className={`absolute top-0 left-0  w-full h-screen bg-white shadow-lg z-1000 transform transition-transform duration-300 rounded-2xl flex flex-col p-5 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                {/* Close button */}
                <div className="flex justify-between items-center p-4 border-b border-gray-300 w-full mb-5">
                ...
                <ul className=" p-4 font-medium space-y-1">
                    {navItems.map(item =>
                        <li key={item.label} onClick={() => setIsMenuOpen(false)} className=' mb-2 font-medium text-lg'><Link href={item.link}>{item.label}</Link></li>
                    )}
                </ul>
            </div>
        </div>
